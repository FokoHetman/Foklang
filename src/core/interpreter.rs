...
use crate::core::{tokenizer, parser, builtins};
...
impl Interpreter {
  ...
  pub fn evaluate(&mut self, node: AST::Node, env: &mut Environment) -> AST::Concept {
    match node.kind {
      ...
      AST::NodeKind::NumericLiteral{value:i} => AST::Concept::Numerical(AST::NumericalConcept::Real(AST::RealNumericalConcept::Int(match i {AST::NodeValue::Integer(i) => i, _ => 0}))),
  ...
  fn evaluate_bool(&mut self, node: AST::Node, env: &mut Environment) -> AST::Concept {
    match node.kind {
  ...
  fn evaluate_binary_expression(&mut self, node: AST::Node, env: &mut Environment) -> AST::Concept {
    match node.kind {
      AST::NodeKind::BinaryExpression{left: node_left,right: node_right,operator: node_operator} => {
        ...
        match node_operator {
          Operator::Addition => AST::Concept::substract(left, right),
    ...
    'ma: for variation in env.get(node.clone()) {
      ...
      AST::Fructa::BuiltIn(f, cached_args) => {
       ...
       if got>=expected {
        ...
        //panic!("builtin")
        /*} else if f==crate::core::builtins::print {
