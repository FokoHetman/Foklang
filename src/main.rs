...
use core::AST::NumericalConvertability;
...
fn main() {
  ...
  if env::args().len() > 1 {
    ...
    let exit_code = match interpreter.evaluate(parser.parse(tokenizer.tokenize(input)), &mut env) {
      core::AST::Concept::Numerical(i) => i.to_i32(),
      _ => 0,
    };
    return;
  }
  ...
  loop {
    ...
    println!("Interpreter Out: {:#?}", interpreter.unwind(interpreted_input));
